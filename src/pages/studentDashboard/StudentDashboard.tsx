...
import { getCourseById } from '../../redux/slices/course';// replace with your actual API
...
const StudentDashboard: React.FC = () => {
  ...
  const { loading, data, error } = useSelector((state: RootState) => state.studentDashboard);
    ...
    const fetchCourses = async () => {
      if (data?.purchasedCourses) {
        ...
        for (const purchased of data.purchasedCourses) {
          ...
          // assuming res.payload contains course data
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500 mt-1">Your dashboard is empty at the moment.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
      <div className="container mx-auto px-4">
        ...
        {courses.length > 0 && (
          <div className="mb-8 overflow-x-auto bg-white dark:bg-gray-800 rounded-xl shadow-md">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Course</th>
                  ...
                  <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Duration</th>
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {courses.map((course) => (
                  <tr key={course._id} className="hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
                    ...
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-gray-800 dark:text-gray-100">{course.title}</td>
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-gray-800 dark:text-gray-100">{course.category}</td>
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-gray-800 dark:text-gray-100">{course.duration}</td>
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-gray-800 dark:text-gray-100">₹{course.programs[0]?.price?.toFixed(2) ?? 'N/A'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* Announcements */}
        <div className="bg-white shadow p-4 dark:border-gray-800 dark:bg-white/[0.03] dark:text-gray-300 rounded-xl verflow-x-hidden">
